  ...
  <div class="chart-card">
    ...
    <div class="chart-summary">
      <div class="summary-item">
        ...
        <span class="summary-value">{{ monthTotal }} {{ $t('admin.dashboard.ordersUnit') }}</span>
      </div>
      <div class="summary-item">
        <span class="summary-label">{{ $t('admin.dashboard.dailyAverage') }}</span>
        <span class="summary-value">{{ dailyAverage.toFixed(1) }} {{ $t('admin.dashboard.ordersUnit') }}</span>
      </div>
    </div>
  </div>
</template>
...
// Calculate daily orders for the selected month
const dailyOrders = computed(() => {
  ...
  const orderCounts = Array(daysInMonth).fill(0);
...
const monthTotal = computed(() => {
  return dailyOrders.value.reduce((sum, val) => sum + val, 0);
});

const dailyAverage = computed(() => {
  const daysWithData = dailyOrders.value.filter((v) => v > 0).length || 1;
...
const chartData = computed(() => {
  ...
  return {
    ...
    datasets: [
      {
        label: t('admin.dashboard.ordersLabel'),
        data: dailyOrders.value,
  ...
  plugins: {
    ...
    tooltip: {
      ...
      callbacks: {
        ...
        label: (item) => `${item.raw} ${t('admin.dashboard.ordersUnit')}`,
      },
    },
  },
  scales: {
